...
public class RecipeService implements RecipeServiceI {
    ...
    public List<RecipeDto> searchByText(String query) {
        if (query == null || query.isBlank()) return repository.findAll()
                .stream().map(RecipeMapper::toDto).collect(Collectors.toList());
        List<RecipeEntity> found = repository.findByNameContainingIgnoreCaseOrCuisineContainingIgnoreCase(query, query);
